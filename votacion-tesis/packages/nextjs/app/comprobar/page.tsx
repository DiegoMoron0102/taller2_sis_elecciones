...
function ComprobarContent() {
  ...
  const verificar = async (e: React.FormEvent) => {
    ...
    router.replace(`/comprobar?txHash=${encodeURIComponent(hash)}`, { scroll: false });
    setEstado("loading");
    setResultado(null);
    setMensajeError(null);

    try {
      const resp = await fetch(`/api/voto/comprobante?txHash=${encodeURIComponent(hash)}`);
  ...
  const limpiar = () => {
    ...
    setResultado(null);
    setMensajeError(null);
    router.replace("/comprobar", { scroll: false });
  ...
  const fechaFormateada = resultado
    ? new Intl.DateTimeFormat("es-BO", {
        dateStyle: "long",
        timeStyle: "medium",
      }).format(new Date(resultado.timestamp * 1000))
    : null;
      ...
      <form onSubmit={verificar} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          ...
          <input
            ...
            spellCheck={false}
            autoComplete="off"
            className="w-full rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-4 py-3 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-[#197fe6] transition-colors"
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button
            ...
            className="px-8 h-11 rounded-xl bg-[#197fe6] text-white font-bold shadow-lg shadow-[#197fe6]/20 hover:brightness-110 transition-[filter,opacity] flex items-center gap-2 disabled:opacity-60"
          >
            {estado === "loading" ? (
              <>
              ...
              <>
                <span aria-hidden="true" className="material-symbols-outlined text-base">search</span>
                Verificar
              </>
            )}
          </button>
      ...
      <div aria-live="polite" aria-atomic="true">
        ...
        {estado === "ok" && resultado && (
          <div className="rounded-xl border border-green-400/40 bg-green-50 dark:bg-green-900/20 p-6 flex flex-col gap-4">
            <div className="flex items-center gap-3">
              <span aria-hidden="true" className="material-symbols-outlined text-green-600 text-4xl">verified</span>
              <div>
            ...
            <div className="flex gap-3 flex-wrap">
              <Link
                href="/explorer"
                className="inline-flex items-center gap-2 px-6 h-10 rounded-xl bg-[#197fe6] text-white font-bold text-sm hover:brightness-110 transition-[filter]"
              >
                <span aria-hidden="true" className="material-symbols-outlined text-base">search</span>
                Ver en Explorer
              </Link>
        ...
        {estado === "notfound" && (
          <div className="rounded-xl border border-red-300/40 bg-red-50 dark:bg-red-900/20 p-6 flex flex-col gap-3">
            <div className="flex items-center gap-3">
              <span aria-hidden="true" className="material-symbols-outlined text-red-500 text-4xl">gpp_bad</span>
              <div>
        ...
        {estado === "error" && (
          <div className="rounded-xl border border-orange-300/40 bg-orange-50 dark:bg-orange-900/20 p-6 flex items-start gap-3">
            <span aria-hidden="true" className="material-symbols-outlined text-orange-500 text-3xl flex-shrink-0">warning</span>
            <div>
      ...
      {estado === "idle" && (
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
          {[
            ...
            >
              <span aria-hidden="true" className="material-symbols-outlined text-2xl text-[#197fe6]">{card.icon}</span>
